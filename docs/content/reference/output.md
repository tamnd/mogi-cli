...
describes what users get. Pick a format with `-o`, or let mogi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mogi <command> -o table   # aligned columns for reading
mogi <command> -o jsonl   # one JSON object per line, for piping
mogi <command> -o json    # a single JSON array
mogi <command> -o csv     # spreadsheet friendly
mogi <command> -o tsv     # tab-separated
mogi <command> -o url     # just the URL column
mogi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
mogi <command> --fields id,title,url
```
...
```bash
mogi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
mogi <command>            # a table, because this is a terminal
mogi <command> | wc -l    # JSONL, because this is a pipe
```
